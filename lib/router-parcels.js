...
router.get('/:expedicion/almacen/soloinfo', function(request, response, next){
	// expedicion must be defined
	if(typeof request.params.expedicion === 'undefined'){
	   response.status(400).json({error : http.STATUS_CODES[400]});
	   return;
	};
    
    // Read Parcel
    response.locals.database.readParcelAlmacen(request.params.expedicion, function(error, parcel){
        if(error){
            response.status(500).json({error : error.message});
        }else{
...
router.get('/:expedicion/almacen/carga', function(request, response, next){
	// expedicion must be defined
	if(typeof request.params.expedicion === 'undefined'){
	   response.status(400).json({error : http.STATUS_CODES[400]});
	   return;
    ...
    // Read Parcel
    response.locals.database.makeCarga(request.params.expedicion, response, function(error, resultMessage){
        if(error){
            response.status(500).json({error : error.message});
        }else{
...
router.post('/:expedicion/almacen/confirmacarga', function(request, response, next){
	// expedicion must be defined
	if(typeof request.params.expedicion === 'undefined'){
	   response.status(400).json({error : http.STATUS_CODES[400]});
	   return;
	...
	// body.cf_itinerario must be defined
	if (!request.body.cf_itinerario){
		response.status(400).json({error : http.STATUS_CODES[400]});
		return;
	}
	
	var confirmaCarga_data = {
		expedicion : request.params.expedicion,
		cf_itinerario : request.body.cf_itinerario,
		cf_agencia : response.locals.user.agency,
		id_repartidor : response.locals.user.id
	};
	
    // Read Parcel
    response.locals.database.confirmaCarga(confirmaCarga_data, function(error, resultMessage){
        if(error){
            response.status(500).json({error : error.message});
        }else{
            response.json(resultMessage);
...
router.get('/:expedicion/almacen/retorno', function(request, response, next){
    // expedicion must be defined
    if(typeof request.params.expedicion === 'undefined'){
        response.status(400).json({error : http.STATUS_CODES[400]});
        return;
    ...
    var numExpedicion = request.params.expedicion;

    // Read Parcel
    response.locals.database.getRetorno(numExpedicion, response, function(error, resultMessage){
        if(error){
            response.status(500).json({error : error.message});
        }else{
            response.json(resultMessage);
...
router.put('/:expedicion/almacen/recepcion', function(request, response, next){
    // expedicion must be defined
    if(typeof request.params.expedicion === 'undefined'){
        response.status(400).json({error : http.STATUS_CODES[400]});
        return;
    ...
    response.locals.database.setRecepcion(numExpedicion, response, function(error, resultMessage){
        if(error){
            response.status(500).json({error : error.message});
        }else{
            response.json(resultMessage);
