...
router.post('/:id/update/driverComments', function(request, response, next){
    if (!request.body.comentarios_repartidor){
        response.status(400).json({error : http.STATUS_CODES[400]});
    } else {
    	var comments = {
            id : request.params.id,
...
router.post('/:id/update/cannotDeliver', function(request, response, next){
    if (	!request.body.id_repartidor 		|| 
    		...
    		!request.body.comentarios_entrega 	|| 
    		!request.body.foto 					|| 
    		!request.body.tono_duracion 		|| 
    		!request.body.telf_duracion 		|| 
    		!request.body.orden_itinerario		|| 
    		!request.body.lat 					|| 
    		!request.body.lon
    ){
...
router.post('/:id/update/postpone', function(request, response, next){
    if (	!request.body.id_repartidor 		|| 
    		...
    		!request.body.horario1_inicio		|| 
    		!request.body.horario1_final		|| 
    		!request.body.horario2_inicio 		|| 
    		!request.body.horario2_final		||
    		!request.body.lat 					|| 
    		!request.body.lon
    ){
    	console.log(request.body);
