	...
	if (fieldNames.length == 0){
		...
		database._query(query, function(error, results){
			if (error){
				...
				javaFile.write('\tpublic enum ' + fieldName + '{\n');
				for (var i = 0; i < results.length; i++){
					var safeName = results[i].descripcion.split(' ').join('_');
					javascriptFile.write('\t\t' + safeName + ': ' + results[i].id2);
					javascriptFile.write((i < results.length - 1)? ',\n' : '\n');
					javaFile.write('\t\t' + safeName + '(' + results[i].id2);
					javaFile.write((i < results.length - 1)? '),\n' : ');\n');
				}
				javascriptFile.write((fieldNames.length > 0)? '\t},\n' : '\t}\n');
				javaFile.write('\n\t\tint value;\n\n');
				javaFile.write('\t\tpublic ' + fieldName + '(int value){\n\t\t\tthis.value = value;\n\t\t}\n\n');
				javaFile.write('\t\tpublic int getValue(){\n\t\t\treturn value;\n\t\t}\n');
				javaFile.write('\t};\n\n');
				recursiveWrite();
			}
		})
