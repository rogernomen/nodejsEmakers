	...
	this._query(queryParcels, function(error, results){
		if(error){
			...
			Object.keys(results).forEach(function(key){
				...
				var arrayFranjas = [];
				if(results_parcels[key].conf_franja == 1){
        ...
        if (error) {
        	...
        	var queryIntento = ""+
        		"INSERT INTO argos_entregas_intentos "+
        			"(id2, cf_agencia, cf_entrega, cf_itinerario, cf_repartidor, cf_estado, fecha_intento, texto_intento, lat, lon) "+
        		"VALUES ("+ 
        			"NULL, "+
	        	...
	        	if (error) {
		        	...
		        	var queryLog = ""+
		        		"INSERT INTO argos_notificaciones_entregas "+
		        			"(id2, cf_entrega, cf_itinerario, cf_repartidor, titulo_notificacion, texto_notificacion, fecha_creacion, lat, lon)"+
		        		"VALUES ("+ 
		        			"NULL, "+
		        			""+mysql.escape(delivery_data.id)+", "+
		        			""+mysql.escape(delivery_data.cf_itinerario)+", "+
...
var updateCannotDeliver = function(cannotDelivery_data, callback){
    ...
    this._query(query, function(error, results){
        if (error) {
        	...
        	var queryIntento = ""+
        		"INSERT INTO argos_entregas_intentos "+
        			"(id2, cf_agencia, cf_entrega, cf_itinerario, cf_repartidor, cf_estado, fecha_intento, lat, lon) "+
        		"VALUES ("+ 
        			"NULL, "+
