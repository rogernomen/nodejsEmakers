...
var makeCarga = function(expedicion, response, callback){
	var self = this;
	// Primero comprovamos la posibilidad de hacer carga valorando la info del pedido
	var queryCheck = ""+
		"SELECT "+
			"e.id2, "+
			"e.cf_itinerario, "+
	...
	this._query(queryCheck, function(error, results){
		if(error){
			...
			// Puede que este en precargas... TODO
			...
			self._query(queryCheckPrecargas, function(error, resultsPrecargas){
				if(error){
	        		...
	        		if(resultsPrecargas[key].cf_franja != 0 && resultsPrecargas[key].cf_franja_premium == 0){
		        		cf_franja = resultsPrecargas[key].cf_franja;
		        		descripcion_franja = resultsPrecargas[key].descripcion_franja;
	        		}else if(resultsPrecargas[key].cf_franja == 0 && resultsPrecargas[key].cf_franja_premium != 0){
		        		cf_franja = resultsPrecargas[key].cf_franja_premium;
		        		descripcion_franja = resultsPrecargas[key].descripcion_franja_premium;
	        		}
