...
var getRetorno = function(numExpedicion, response, callback){
    ...
    var querySelect = ""+
        "SELECT "+
        	...
        	"e.notas, "+
        	"DATE_FORMAT(e.fecha_entrega_cliente, '%Y-%m-%d') AS 'fecha_entrega_cliente', "+
        	"e.comentarios_repartidor, "+
        	"aee.descripcion AS 'estado_descripcion', "+
        	"a.nombre_agencia, "+
        	"afe.descripcion AS desc_franja "+
        ...
        "WHERE "+
        	"e.expedicion = "+mysql.escape(numExpedicion)+" AND "+
	        "aee.idioma = '1'";
    ...
    self._query(querySelect, function(error, resultQuerySelect){
        if(error){
            ...
            var queryToUpdate = ""+
            	"UPDATE argos_entregas "+
            		"SET cf_itinerario = 0 "+
            	"WHERE "+
            		"expedicion = "+mysql.escape(numExpedicion)+"";
            ...
            self._query(queryToUpdate, function (error, result) {
				if(error) {
					...
					var queryLog = ""+
						...
						"VALUES ("+
							"NULL, "+
							""+resultQuerySelect[0].id2+", "+
							""+mysql.escape(response.locals.user.id_repartidor)+", "+
							"'Expedicion "+numExpedicion+" retorna', "+
							"'Expedicion "+numExpedicion+" - "+response.locals.user.nombre_repartidor+" retorna a SIN ASIGNAR', "+
							"now())";
					...
					self._query(queryLog, function(error, resultLog){
						if (error) {
							...
							var retorno_response = {
								expedicion: resultQuerySelect[0].expedicion,
								cf_estado: resultQuerySelect[0].cf_estado,
								descripcion_estado: resultQuerySelect[0].estado_descripcion,
								cf_itinerario: "0",
								...
								comentarios_repartidor: resultQuerySelect[0].comentarios_repartidor
							};
							callback(null, retorno_response);
						}
					});
				}
            });
...
var setRecepcion = function(numExpedicion, response, callback){
    ...
    var selectPrecargas = ""+
        ...
        "FROM argos_entregas_webservice as e "+
       		"LEFT JOIN argos_agencias as a on a.id2 = e.cf_agencia_destino "+
        ...
        if(error){
            ...
            if(resultPrecargasSelect.length > 0) {
                _checkRecepcion(self,
                    ...
                    resultPrecargasSelect[0].otros_operadores,
                    response.locals.user.agency,
                    callback,
                    true);
            } else {
                // NO EXISTE EN PRECARGAS
                var selectExpediciones = ""+
                	"SELECT "+
                		"e.id2, "+
                		"e.cf_agencia_destino, "+
                		"e.cf_agencia_origen, "+
                    ...
                    "WHERE "+
                    	"expedicion = "+mysql.escape(numExpedicion)+" AND "+
                    	"e.ifActiu = 1";
                ...
                self._query(selectExpediciones, function (error, resultExpedicionesSelect) {
                    if(error) {
                        ...
                        if(resultExpedicionesSelect.length > 0) {
                            _checkRecepcion(self,
                            	response,
                            	resultExpedicionesSelect[0].id2,
                                numExpedicion,
                                ...
                                resultExpedicionesSelect[0].otros_operadores,
                                response.locals.user.agency,
                                callback,
                                false);
                        } else {
                            var recepcion_response = {
			                    recepcionado        : 0,
			                    cf_agencia_destino  : 0,
			                    nombre_agencia      : 'NO ENCONTRADO',
			                    estado              : '',
			                    cf_estado           : 0
			                };
...
var _checkRecepcion = function (self, response, cf_entrega, numExpedicion, nombreAgencia, agencia_origen, agencia_destino, otros_operadores, agencia_usuario, callback, precargas) {

    var table       = precargas ? 'argos_entregas_webservice' : 'argos_entregas',
        updateQuery = "UPDATE " + table + " SET cf_estado = ";

    if(otros_operadores == 1){
	    nombreAgencia = 'CORREOS';
    }
	            ...
	            if(table == 'argos_entregas'){
					...
					var queryLog = ""+
						...
						"VALUES ("+
							...
							""+cf_entrega+", "+
							""+mysql.escape(response.locals.user.id_repartidor)+", "+
							"'Expedicion "+numExpedicion+" se recepciona', "+
							"'Expedicion "+numExpedicion+" - "+response.locals.user.nombre_repartidor+" recepciona a EN ALMACEN DESTINO', "+
							"now())";
					...
					self._query(queryLog, function(error, resultLog){
						if (error) {
							...
							// Insertamos el registro del intento
							// Pedido EN RUTA, registramos intento y log antes de responder
							var queryIntento = ""+
				        		"INSERT INTO argos_entregas_intentos "+
				        			"(id2, cf_agencia, cf_entrega, cf_itinerario, cf_repartidor, cf_estado, fecha_intento) "+
				        		"VALUES ("+ 
				        			"NULL, "+
				        			""+mysql.escape(agencia_destino)+", "+
				        			""+mysql.escape(cf_entrega)+", "+
				        			"0, "+
				        			""+mysql.escape(response.locals.user.id)+", "+
				        			""+mysql.escape(estado)+", "+
				        			"now()"+
            	...
            	if(table == 'argos_entregas'){
					...
					var queryLog = ""+
						...
						"VALUES ("+
							...
							""+mysql.escape(response.locals.user.id_repartidor)+", "+
							"'Expedicion "+numExpedicion+" se recepciona', "+
							"'Expedicion "+numExpedicion+" - "+response.locals.user.nombre_repartidor+" recepciona a EN ALMACEN ORIGEN', "+
							"now())";
