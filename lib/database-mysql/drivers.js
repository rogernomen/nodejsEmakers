...
var readAvatarDriver = function(ids, callback){
	var query = "SELECT id2, img_repartidor_jpg_base64, '' AS 'img_base64' FROM argos_repartidores"; //, CONVERT(img_repartidor_jpg_base64 USING utf8) AS 'img_base64' 
	if (!callback){
        callback = ids;
    } else {
        query += ' WHERE id2 IN (';
    ...
    this._query(query, function(error, results){
        if (error){
        	...
        	for (var i = 0; i < results.length; i++){
	            results[i].img_base64 = new Buffer(results[i].img_repartidor_jpg_base64).toString('base64');
	        }
...
var updateDriverLogin = function(request, response, callback){
	...
	this._query(queryUpdate, function(error, resultUpdate){
        if (error){
        	...
        	// Actualizamos tambien los mismos datos en repartidores
        	var queryUpdateRepartidores = ""+
        		"UPDATE argos_repartidores SET "+
        			"telf_repartidor2 = "+mysql.escape(request.body.telf_repartidor)+", "+
        			"hash_dispositivo = "+mysql.escape(request.body.hash_dispositivo)+", "+
        			"mac_lector = "+mysql.escape(request.body.mac_lector)+" "+ 
        		"WHERE "+
        			"id2 = "+mysql.escape(response.locals.user.id);
            
            // Execute query
            self._query(queryUpdateRepartidores, function(error, resultUpdateRepartidores){
	            if( error ){
		            callback(error);
	            }else{
