...
var readAvatarDriver = function(ids, callback){
	var query = "SELECT id2, img_repartidor_jpg_base64 AS 'img_base64', img_repartidor_base64 FROM argos_repartidores"; //CONVERT(img_repartidor_base64 USING utf8) AS 'img_base64'
	if (!callback){
        callback = ids;
    } else {
        query += ' WHERE id2 IN (';
