							...
							self._query(queryParcels_by_Route, function(error, results_parcels){
								if(error){
									...
									Object.keys(results_parcels).forEach(function(key){
										...
										if(results_parcels[key].cf_tipo_servicio == self.constants.ServiceTypes.RETURN){
											if(results_parcels[key].fase == 2  && results_parcels[key].cf_estado != self.constants.ParcelStatus.COLLECTED){
										...
										//console.log(franjas[idioma][conf_franja]);
										//results_parcels[key]['franjas_aplicables'] = [ { id2: 1, descripcion: 'MAÑANA' }, { id2: 2, descripcion: 'TARDE' }, { id2: 4, descripcion: 'DIURNA' }, { id2: 3, descripcion: 'NOCHE' } ];
